...
#define Nk      16
#define Nr      22
#define KEY_EXP_SIZE 368

// Object for encryption/decryption.
struct RES_ctx_CBC_512 {
...
inline static void RES_init_ctx_iv(struct RES_ctx_CBC_512 *ctx, const uint8_t *key, const uint8_t *iv) {
...
// Encrypt operation.
inline static void RES_CBC_encrypt_buffer(struct RES_ctx_CBC_512 *ctx, uint8_t *buf, uint32_t length) {
    uintptr_t i;
    uint8_t *Iv = ctx->Iv;
...
// Decrypt operation.
inline static void RES_CBC_decrypt_buffer(struct RES_ctx_CBC_512 *ctx, uint8_t *buf,  uint32_t length) {
    uintptr_t i;
