...
inline static void RES_init_ctx(struct RES_ctx_ECB_512 *ctx, const uint8_t *key) {
...
// Encrypt operation.
inline static void RES_ECB_encrypt(const struct RES_ctx_ECB_512 *ctx, uint8_t *buf) {
    Cipher((state_t *) buf, ctx->RoundKey);
}

// Decrypt operation.
inline static void RES_ECB_decrypt(const struct RES_ctx_ECB_512 *ctx, uint8_t *buf) {
    InvCipher((state_t *) buf, ctx->RoundKey);
}

/**
 * @brief Red::EncryptResECB512
 *
